...
//    public static final String tbName = "zgpn";
    public static final String tbName = "zgdm";
    public static final String tmpTbName = "pn_temp";
    //    public static String[] columns = null;
    public static final String dic_TbName = "sys_dict";

    public static String[] columns = new String[]{
            "id", "name", "地名代码", "BSM", "BZBSM", "大类", "小类", "类别名称", "position", "X", "Y",
            "spaType", "path", "spaTypeName", "TBNAME", "DMCX", "LayerName", "简称", "别名", "曾用名",
            "地名原读音", "汉语普通话读音", "罗马字母拼写", "少数民族语书写", "地名语种", "地名普查状态",
            "原图名称", "图号（年版）", "比例尺", "使用时间", "地名的历史沿革", "东经", "至东经", "北纬", "至北纬",
            "密级", "坐标系", "测量方法", "地理实体概况", "多媒体信息", "资料来源", "备注", "登记时间", "登记人",
            "登记单位", "通名罗马字母拼写", "设立年份", "废止年份", "DMID", "数据来源", "地名的含义", "地名来历",
            "审核人", "HZSX", "dist", "citycode", "所在跨行政区", "brif", "desbrif",

    };
    //  简单式地名数据，只导出以下列
    public static String[] easyColumnNames = new String[]{
            "id", "name", "别名", "大类", "小类", "类别名称", "position", "spaType", "path",
            "所在跨行政区", "dist", "citycode", "汉语普通话读音", "简称", "brif", "desbrif", "多媒体信息", "DMID"
    };

    //  获取所有地名的json
    public static String getTotalGeonameInfo(){
//        String sql = "SELECT * from " + tbName;
        String sql = "SELECT * from " + "zgpn_copy1";
...
//        String str = PlaceJson.toJson(ps, easyColumnNames);
    ...
    public static String getEasyGeonamesByWords(String words_str) {
        String words_clear = words_str.replace("+", "").replace("*", "").replace("&", "").
                replace("%", "").replace("$", "").replace("`", "");
        String[] words_split = words_clear.split(" ");
        List<String> words = new LinkedList<>();
        for(String s_word : words_split) {
            if(s_word.length() < 1 || s_word.equals(" ")) {
                continue;
            }
            words.add(s_word);
        }
        String cns = MySQLQuery.createSqlColumns(easyColumnNames);
        String sql = "SELECT " + cns + " from " + tbName + " WHERE `name` like '%" + words_clear + "%'";
        if (words.size() > 1) {
            sql = "SELECT " + cns + " from " + tbName + " WHERE ";
            for (int i = 0; i < words.size() - 1; i++) {
                String word = words.get(i);
                sql = sql + " `name` like '%" + word + "%' AND ";
            }
            String word = words.get(words.size() - 1);
            sql = sql + " `name` like '%" + word + "%'";
        }
        List<PlaceJson> ps = searchPlaces(sql);
        if (ps.size() < 1) {
            if(words.size() > 1) {
    ...
    public static String getGeonameInfoByNickname(String val, boolean admin){
        ...
        if(admin) {
            ...
            sql = "SELECT * from " + tmpTbName + " where `别名` = '" + val +
                    "' or `汉语普通话读音` = '" + val + "' or `name` = '" + val + "'";
        } else {
            PlaceJson.consColumnNames(dbType, tbName);
            sql = "SELECT * from " + tbName + " where `别名` = '" + val +
                    "' or `汉语普通话读音` = '" + val + "' or `name` = '" + val + "'";
        }
    ...
    public static int updatePlace(Map<String, String[]> reqParas) {
        int id = -100;
        Map<String, Object> paras = new HashMap<>();
        List<String> keys = new LinkedList<>();
        List<Object> vals = new LinkedList<>();
        try {
            if (reqParas.containsKey("id")) {
                String idStr = reqParas.get("id")[0];
                id = Integer.parseInt(idStr);
            }
            for (String _key : reqParas.keySet()) {
                String _val = reqParas.get(_key)[0];
                if ("id".equals(_key)) {
                    continue;
                }
                if ("X".equalsIgnoreCase(_key) || "Y".equalsIgnoreCase(_key)) {
                    float num = Float.parseFloat(_val);
                    paras.put(_key, num);
                    keys.add(_key);
                    vals.add(num);
                } else {
        ...
        String sql = "";
        if(id == -100) {    //  insert
            String sql1 = "INSERT INTO " + tbName + " (", sql2 = " values(";
