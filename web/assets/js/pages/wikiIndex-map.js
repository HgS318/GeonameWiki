...
function createWordQuery(keyword) {
    var sql = "name LIKE '%" + keyword + "%'";
    try {
        var words_clear = keyword;
        while (words_clear.indexOf("+") > -1) {
        ...
        var words_split = words_clear.split(" ");
        var words = [];
        for (var i = 0; i < words_split.length; i++) {
            var s_word = words_split[i];
            if (s_word.length < 1 || " " == s_word) {
                continue;
            }
            words.push(s_word);
        }
...
function search_keyword() {
    ...
    if(keyword != undefined && keyword != null) {
        var sql = createWordQuery(keyword);
    ...
    document.getElementById("backAll").style.display = "none";
}
...
function gotoBigType(bigtype) {
    ...
    if("" == bigtype || "所有分类" == bigtype) {
        document.getElementById("backAll").style.display = "none";
    } else {
        document.getElementById("backAll").style.display = "block";
    }
}
...
function gotoSmallType(bigtype, smalltype) {
    ...
    if("" == bigtype || "所有分类" == bigtype || "" == smalltype || "所有分类" == smalltype) {
        document.getElementById("backAll").style.display = "none";
    } else {
        document.getElementById("backAll").style.display = "block";
    }
...
function create_popup(feature) {
    ...
    if(pic_info === undefined || pic_info == null || "" == pic_info) {
        pic_info = extData['name'] + "11.jpg";
    } else if(pic_info.indexOf('.jpg') <0 && pic_info.indexOf('.JPG')) {
        pic_info = extData['name'] + "11.jpg";
    }
    pic_info = pic_info.replace(".JPG", ".jpg");
    var img_src = "data/media/201803/mini/" + pic_info;
    // var img_src = "images/contentdemopic.jpg";

    var brif = extData['brif'], brifObj = "";
    if(brif) {
        brifObj = brif.replace("<br/>", "  ");
        if(brifObj.length > 34) {
            brifObj = brifObj.substring(0, 32) + "...";
        }
    }
    content = content + ("<img src='" + img_src +
        "' onerror='this.src=\"images/contentdemopic.jpg\"' />"
        + "<strong>地名含义：</strong>" + brifObj + "<br/>");
    ...
    content = content + ("<strong>地理实体描述：</strong>" + showobj + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
        ...
        "<a href='html/wikiContent_fitall.html?zgid=" + extData['dmid'] + "' target='_blank'>详细信息</a>");
