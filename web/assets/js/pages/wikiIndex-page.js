...
function gotoPage(page, feature_list) {
    //  根据页码得到相应的结果要素，并填充到左侧（或右侧）结果列表中
    close_popup();
    if(feature_list === undefined || feature_list == null || feature_list.length < 1) {
        feature_list = result_features;
    } else {
        result_features = feature_list;
    }
    var num_result = feature_list.length;
