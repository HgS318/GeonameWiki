...
$(function() {
    ...
    $.ajax({
        ...
        success: function (_data) {
            ...
            if(_data['spaType'] === undefined || _data['spaType'] == null) {
                _data['spaType'] = 1;
            }
            data = _data;
            orgData = _data;
...
function mapInit(data) {
    AMapUI.loadUI(['control/BasicControl'], function(BasicControl) {
        ...
        consDivCotent(data, "X", "xpos");
        consDivCotent(data, "Y", "ypos");
        consDivCotent(data, "path", "pathtext");
...
function markerDrag(e) {
    ...
    var y = pos.lat;
    var tmpdata = {"X": x, "Y": y};
    consDivCotent(tmpdata, "X", "xpos");
    consDivCotent(tmpdata, "Y", "ypos");
}
...
function lineEdit() {
    ...
    consDivCotent(tmpdata, "path", "pathtext");
}
...
function consDivCotent(data, attaname, divname) {
    if(data[attaname]) {
...
function consBasicCotent(data, attaname, divname) {
    var ele = document.getElementById("td_" + divname);
    var val = "";
    if(data[attaname]) {
        // document.getElementById(divname).value = data[attaname];
        // var val = data[attaname];
        // ele.value = val;
        val = '<input id="' + attaname + '" size="30" value="' + data[attaname] + '"/>';
        ele.innerHTML = val;
    } else {
        ele.value = val;
    }
    basic_fileds.push(attaname);
}

function add_table_val(field_name, field_value){
    var tab = document.getElementById("attr_tb");
    var oneRow = tab.insertRow();
    var cell1 = oneRow.insertCell();
    var cell2 = oneRow.insertCell();
    cell1.innerHTML = "&nbsp; " + field_name;
    var cell_val = '<input id="' + field_name + '" size="30" value="' + field_value + '"/>';
    cell2.innerHTML = cell_val;
    basic_fileds.push(field_name)
}
...
//         document.getElementById(divname).innerHTML ="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + str;
        document.getElementById(divname).innerText = org;
...
function getAllEditInfo() {
    var info = {
        "name": getEditInfo("newplacename"),
        "地名含义": getEditInfo("hanyi"),
        "地名来历": getEditInfo("laili"),
        "历史沿革": getEditInfo("yange"),
        "地理实体描述": getEditInfo("miaoshu"),
        "资料来源及出处": getEditInfo("ziliao"),
        // "X": getEditInfo("xpos"),
        // "Y": getEditInfo("ypos"),
        "大类": getEditInfo("bigtype"),
        "小类": getEditInfo("smalltype"),
        // "": getEditInfo(""),
        // "": getEditInfo(""),
    };
    info['类别名称'] = info['小类'];
    for(var i = 0; i < basic_fileds.length; i++) {
        var field_name = basic_fileds[i];
        info[field_name] = getEditInfo(field_name);
    }
    var xStr = getEditInfo("xpos");
    if(xStr != "") {
        try {
    ...
    if('X' in info) {
        info['position'] = info['X'] + "," + info['Y'];
    }
    return info;
    //  hanyi/laili/yange/miaoshu/ziliao/xpos/ypos/bigtype/smalltype
}
...
function trimText(text) {
    ...
    try {
        var trim_str = text.trim();
        var clear_str = trim_str.replace(/\"/g,"\'");
        return clear_str;
    } catch (exp) {
        return text;
    }
...
function getModifiedInfo() {
    var nowInfo = getAllEditInfo();
    var modInfo = {};
    for(var field in nowInfo) {
        if(field in orgData) {
            if(orgData[field] != nowInfo[field]) {
                modInfo[field] = nowInfo[field];
            }
        } else {
            var text = trimText(nowInfo[field]);
            if("" != text) {
                modInfo[field] = text;
            }
        }
...
function modifiedQuery() {
    var modInfo = getModifiedInfo();
    var paramsq = "";
    for(var field in modInfo) {
        var trimVal = trimText(modInfo[field]);
        paramsq = paramsq + field + "=" + trimVal + "&";
    }
    if(paramsq.length < 2) {
        return null;
    }
    var query = "updateWikiPlace.action?" + paramsq.substring(0, paramsq.length - 1);
    return query;
}

function commitEdit() {
    var url = modifiedQuery();
    $.ajax({
        url: url,
        type: 'get',
        dataType: 'json',
        success: function (_data) {
            if("status" in _data) {
                if(_data["status"] == 1) {
                    alert("地名更新成功！");
                    var id_str = getQueryString("zgid"), url = "";
                    if("" == id_str) {
                        var placename = getQueryString("name");
                        url = "wikiContent_fitall.html?name=" + placename;
                    } else {
                        url = "wikiContent_fitall.html?zgid=" + id_str;
                    }
